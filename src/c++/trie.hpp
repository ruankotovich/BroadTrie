...
#define IGNORE_WORD_LENGTH 3
    ...
    struct TrieResponse_t {
        ...
        TrieResponse_t(const T *_content, int _editDistance)
                : content(_content), editDistance(_editDistance) {
        }
    };
    ...
    public:
        TrieNode_t(unsigned int val)
                : m_content(val), m_endOfWord(false) {
        }
    ...
    struct ActiveNode_t {
        ...
        ActiveNode_t(TrieNode_t<T> *nd, int ed)
                : node(nd), editDistance(ed), positionDistance(0) {
        }

        ActiveNode_t(TrieNode_t<T> *nd, int ed, int pos)
                : node(nd), editDistance(ed), positionDistance(pos) {
        }
    ...
    class Trie_t {
        ...
        std::map<unsigned int, unsigned int> m_characterMap; // used to map all the characters to it's defined codes
        std::map<unsigned int, char> m_reverseCharacterMap; // used to map all the defined codes to it's characters (4fun)
    ...
    public:
        ...
        template<typename... Args>
        void emplaceIndividualWord(std::string &&str, const Args &... args) {
            emplaceIndividualWord(str, args...);
        }

        void buildActiveNodeSet(bool _onlyFinalWords) {
            std::queue<std::pair<TrieNode_t < T> *, int>>
            seekQueue;
            std::map<TrieNode_t < T>*, TrieNode_t < T > * > father;
...
//            std::cout << std::endl;
        ...
        void encodeCharacters(const std::string &filename) {
            std::cout << "Encoding characters." << std::endl;
        ...
        Trie_t()
                : m_searchLimitThreshold(5), m_fuzzyLimitThreshold(1) {
...
//        encodeCharacters("charmap.cm");
        ...
        std::priority_queue<TrieResponse_t<T>, std::vector<TrieResponse_t<T>>, TrieResponseComparator_t<T>> autocomplete(std::string &keyword) {
            std::priority_queue<TrieResponse_t<T>, std::vector<TrieResponse_t<T>>, TrieResponseComparator_t < T>>
            ocurrencesQueue;
            ...
            std::set<ActiveNode_t < T>>
            lastActiveNodes = this->m_activeNodeSet;
            ...
            std::set<TrieNode_t < T>*> visitedNode;
